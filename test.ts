...
import { installWmicu, checkShell, homeBinOnPath } from "./extensions/index.ts";
...
console.log("installWmicu:");
const dest = path.join(fakeHome, "bin", "wmicu");
const pkgCopy = fs.readFileSync(path.join(pkgRoot, "bin", "wmicu"), "utf8");
let r = installWmicu(pkgRoot, fakeHome);
ok("fresh install", r.status === "installed" && fs.readFileSync(dest, "utf8").includes("pi-git-bash-only: managed"));
r = installWmicu(pkgRoot, fakeHome);
ok("kept when identical", r.status === "kept");
fs.writeFileSync(dest, pkgCopy.replace('wmic "$@"', "# old\nwmic \"$@\""));
r = installWmicu(pkgRoot, fakeHome);
ok("update managed old version", fs.readFileSync(dest, "utf8") === pkgCopy);
fs.writeFileSync(dest, "#!/bin/bash\n# my own tool\nexit 1\n");
r = installWmicu(pkgRoot, fakeHome);
ok("user file conflict untouched", r.status === "conflict" && fs.readFileSync(dest, "utf8").includes("my own tool"));
